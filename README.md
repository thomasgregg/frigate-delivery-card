...
- **Inline clip playback** — a &#9654; button swaps the image for the event's full-quality recorded clip, streamed progressively right inside the card: playback starts within a couple of seconds and plays at full resolution. The scrubber covers the buffered portion (the HA proxy doesn't support range requests, so jumping far ahead isn't possible). &#10005; or clip end returns to the image (requires `record:` enabled in Frigate; hide with `clips: false`)
