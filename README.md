...
- **Inline clip playback** — a &#9654; button swaps the image for the event's recorded clip, playing right inside the card. Playback starts instantly as a stream while the full clip buffers in the background; once buffered it is swapped in seamlessly, giving the player its real duration and instant seeking. &#10005; or clip end returns to the image (requires `record:` enabled in Frigate; hide with `clips: false`)
