...
- **Unrecognized stops** (optional, `unrecognized: true`) — also show vehicles that stopped for 30+ seconds without any courier logo, as a gray "unrecognized" category. Catches unbranded subcontractor vans (looking at you, Amazon DSPs) that no logo model can identify. Tune with `unrecognized_min_duration: <seconds>`
