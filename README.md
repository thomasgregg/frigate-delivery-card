...
- **Inline clip playback** — a &#9654; button swaps the image for the event's recorded clip, playing right inside the card. Clips play via Frigate's HLS recordings (hls.js, native on Safari): the real duration shows immediately, seeking fetches only the segments you jump to, and playback stays smooth even over slow remote connections such as Nabu Casa. Falls back to progressive streaming if HLS is unavailable. &#10005; or clip end returns to the image (requires `record:` enabled in Frigate; hide with `clips: false`)
