...
- **Inline clip playback** — a &#9654; button plays the event's **preview** right inside the card: Frigate's low-res fast-forward render of the whole event (~100 KB), which loads in under a second even on slow remote connections and shows the entire delivery in a few seconds. The raw preview is a ~1 fps timelapse that would race by; the card decodes it and replays it slowed down to a watchable pace (3&#215; slower by default; tune with `preview_slowdown: <factor>` in YAML; browsers without the ImageDecoder API get the raw speed). An **HD** button switches to the full-quality recording, streamed via Frigate's HLS VOD (hls.js, native on Safari) with real duration and instant seeking — best used on your home network, since full-quality bitrate must fit your connection. &#10005; or clip end returns to the image (requires `record:` enabled in Frigate; hide with `clips: false`)
